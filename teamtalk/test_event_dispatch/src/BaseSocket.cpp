...
typedef map<net_handle_t, CBaseSocket*> SocketMap;
...
{
	//log("CBaseSocket::OnRead on %s:%d", m_local_ip.c_str(), m_local_port);
...
{
	//log("CBaseSocket::OnWrite on %s:%d", m_local_ip.c_str(), m_local_port);
#if ((defined _WIN32) || (defined __APPLE__))
...
{
	//log("CBaseSocket::OnClose on %s:%d", m_local_ip.c_str(), m_local_port);
