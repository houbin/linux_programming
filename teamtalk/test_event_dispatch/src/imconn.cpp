...
CImConn::CImConn()
{
	...
	m_last_send_tick = m_last_recv_tick = get_tick();
}
...
{
	m_last_send_tick = get_tick();
//	++g_send_pkt_cnt;

	if (m_busy)
	{
		m_out_buf.Write((const char *)data, len);
		return len;
	}
...
void CImConn::OnRead()
{
	...
	for (;;)
	{
		...
		m_last_recv_tick = get_tick();
	}
    
    // send back
    Send(m_in_buf.GetBuffer(), m_in_buf.GetWriteOffset());
